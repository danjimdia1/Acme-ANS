	...
	public boolean isValid(final Service service, final ConstraintValidatorContext context) {
		...
		if (service == null)
			...
			if (service.getPromotionCode() != null && service.getPromotionCode().length() >= 2) {
				...
				Optional<Service> foundService = SpringHelper.getBean(ServiceRepository.class).findByPromotionCodeAndNotServiceId(service.getPromotionCode(), service.getId());
				Boolean repeatedPromotionCode = foundService.isPresent();
				if (repeatedPromotionCode)
